...
## Phase-1 (Academic Demo): Offline Threat Detection Dashboard

This repository includes a Phase-1 Streamlit dashboard designed for CIC-IDS-style flow CSVs.

### Start the Phase-1 Dashboard (Windows)

Recommended (does not require PowerShell venv activation):
...
The script will:
- use `venv\Scripts\python.exe` if present
- auto-select a free port
- open the Phase-1 dashboard entrypoint
